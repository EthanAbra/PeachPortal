...
function generateSvgPath(data) {
    ...
    let svgPath = `<path style="stroke:${randcolor}" class="chart-line" d="`
    let startCP;
    let endCP;
    data.forEach((dot, i) => {
        if (i !== 0) {
...
const addLineToSVG = (data, multiplier, current) => {
...
window.setInterval(() => {
    for (let i = 0; i < 8; i++){
        ...
        addLineToSVG(line1Data, multiplier, current);
    }
    $el = $(`<svg overflow ="visible" width = "200" height = "200" viewBox = "0 0 400 400" class="svg-container"><g id='chart-container` + containerCount + `'></g></svg>`)
    $('#svgBox')
        .append($el)
        .animate({ height: "+=" + $(window).height()/($(window).width()/45) }, 750, function() {
            $el.fadeIn(250);
          });
